...
public class PielView extends View {
    ...
    public void setData(List<LuckyItem> luckyItemList, Context context) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Log.d("Spinning "," on draw called ");

        for(int i = 0; i < mLuckyItemList.size(); i++) {
            mArcPaint.setColor(mLuckyItemList.get(i).color);
           canvas.drawArc(mRange, tmpAngle, sweepAngle, true, mArcPaint);
//            drawText(canvas, tmpAngle, sweepAngle, mLuckyItemList.get(i).text, BitmapFactory.decodeResource(getResources(), mLuckyItemList.get(i).icon));
            drawImage(i,canvas, tmpAngle,mLuckyItemList.get(i).imageBitmap,mLuckyItemList.get(i).icon);
            tmpAngle += sweepAngle;
        }
...
//        drawCenterImage(canvas, drawableCenterImage);
     ...
     * @param bitmap
     */
    private void drawImage(int position,final Canvas canvas, float tmpAngle, final Bitmap bitmap, int icon) {
        ...
        final Rect rect = new Rect(x - imgWidth/2, y - imgWidth/2, x + imgWidth/2, y + imgWidth/2);
     ...
     * @param mStr
     */
    private void drawText(Canvas canvas, float tmpAngle, float sweepAngle, String mStr, Bitmap bitmap) {
        Path path = new Path();
    ...
    public void rotateTo(int index) {
        ...
        Log.d("target angle",targetAngle+" ");
        animate()
                .setInterpolator(new DecelerateInterpolator())
                .setDuration(mRoundOfNumber * 300 + 2000L)
